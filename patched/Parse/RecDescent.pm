...
use Text::Balanced
  qw ( extract_codeblock extract_bracketed extract_quotelike extract_delimited );
              ...
              #    perl -MParse::RecDescent - <grammarfile> <classname>
{
    ...
    if ( substr( $file, 0, 1 ) eq '-' && $line == 0 ) {
        ...
        my ( $sourcefile, $class, $mod_version ) = @ARGV;
...
sub Precompile {
    my ( $self, $grammar, $class, $sourcefile, $class_version ) = @_;
    $class_version ||= $Parse::RecDescent::VERSION;
    ...
    foreach ( keys %{ $self->{rules} } ) { $self->{rules}{$_}{changed} = 1 }

    print OUT <<CODE_START;
package $class;
...
$class - precompiled Parse::RecDescent grammar package

=head1 DO NOT EDIT

It was generated by B<Parse::RecDescent> from F<${sourcefile}>. Don't edit this file, edit F<${sourcefile}> instead.
...
sub FETCH {
    ...
    my $from   =
      $parser->{fulltextlen} - length( ${ $_[0]->{text} } ) - $_[0]->{prev};
...
sub resync    # ($linecounter)
{
    ...
    my $apparently =
      $parser->{offsetlinenum} -
      Parse::RecDescent::_linecount( ${ $self->{text} } ) + 1;
...
sub FETCH {
    my $parser  = $_[0]->{parser};
    my $missing =
      $parser->{fulltextlen} - length( ${ $_[0]->{text} } ) - $_[0]->{prev} + 1;
...
sub nextoperator($) {
    ...
    return
      "_operator_${opcount}_of_production_${prodcount}_of_rule_$self->{name}";
}
...
sub checkleftmost($) {
    my @items = @{ $_[0]->{"items"} };
    if (   @items == 1
        && ref( $items[0] ) =~ /\AParse::RecDescent::Error/
        && $items[0]->{commitonly} )
    {
        Parse::RecDescent::_warn(
            2, "Lone <error?> in production treated
					    as <error?> <reject>"
        );
        Parse::RecDescent::_hint(
            "A production consisting of a single
        ...
        && ( $items[0]->describe || "" ) =~ /<rulevar|<autoscore/ )
    {
      ...
      )
    {
        Parse::RecDescent::_warn( 1,
            "Optimizing away production: [" . $_[0]->describe . "]" );
        my $what =
        ...
        Parse::RecDescent::_hint(
            "The production starts with $what. That means that the
...
sub addscore {
    ...
    $self->additem(
        Parse::RecDescent::Directive->new(
            "local \$^W;
...
sub check_pending {
    ...
    if ( $self->{op} ) {
        while ( my $next = pop @{ $self->{op} } ) {
            Parse::RecDescent::_error( "Incomplete <$next->{type}op:...>.",
                $line );
            Parse::RecDescent::_hint(
...
sub enddirective {
    ...
    if ( $op->{type} =~ /left|right/ ) {
        if ( $span != 3 ) {
            ...
            push @{ $self->{items} },
              Parse::RecDescent::Operator->new( $op->{type}, $minrep, $maxrep,
                splice( @{ $self->{"items"} }, -3 ) );
...
sub prevwasreturn {
    ...
    push @{ $self->{items} }, Parse::RecDescent::Result->new();
}
...
' if $parser->{_check}{itempos};
        ...
        && !$item->isa("Parse::RecDescent::Action") )
    {
        ...
        Parse::RecDescent::_warn(
            1, "Autogenerating tree-building action in rule
					   \"$rule->{name}\""
          )
          and Parse::RecDescent::_hint(
            "The directive <autotree> was specified,
...
sub describe {
    $_[1] ? '' : $_[0]->{commitonly} ? '<error?:...>' : '<error...>';
}
...
#WAS: $action .= "Parse::RecDescent::_error(qq{$self->{msg}}" .  ',$thisline);';
    ...
    my $dir = new Parse::RecDescent::Directive(
        'if ('
          . ( $self->{"commitonly"} ? '$commit' : '1' )
          . ") { do {$action} unless "
          . ' $_noactions; undef } else {0}',
...
sub sethashname {
    $_[0]->{hashname} = '__PATTERN' . ++$_[1]->{patcount} . '__';
}
...
sub new ($$$$$$) {
    ...
    {
        Parse::RecDescent::_warn(
            3, "Token pattern \"m$ldel$pattern$rdel\"
...
sub code($$$$) {
    ...
    my $code = '
      ...
      . $self->describe . ']}, Parse::RecDescent::_tracefirst($text),
					  q{' . $rule->{name} . '},
					  $tracelevel)
						if defined $::RD_TRACE;
		$lastsep = "";
      ...
      . (
        $check->{itempos}
        ? 'do {' . Parse::RecDescent::Production::incitempos() . ' 1} and '
        : ''
      )
      . '  $text =~ s'
      . $ldel . '\A(?:'
...
sub callsyntax($$) {
    ...
    else { return "\\&$_[1]$_[0]->{subrule}"; }
}

sub new ($$$$$$$$$$) {
    my (
        $self,      $subrule, $repspec, $min,       $max,
        $lookahead, $line,    $parser,  $matchrule, $argcode
      )
      = @_;
    my $class = ref($self) || $self;
    ...
    if ($lookahead) {
        if ( $min > 0 ) {
            return new Parse::RecDescent::Subrule( $subrule, $lookahead, $line,
                $desc, $matchrule, $argcode );
        }
        else {
            Parse::RecDescent::_error(
                "Not symbol (\"!\") before
            ...
            Parse::RecDescent::_hint(
                "Lookahead for negated optional
					   repetitions (such as
...
sub code($$$$) {
    ...
    '
      ...
      . $self->callsyntax( $namespace . '::' ) . ', ' . $min . ', ' . $max
      . ', '
      . ( $self->{"lookahead"} ? '1' : '$_noactions' )
      . ',$expectation,'
      . (
...
sub code($$$$) {
    ...
    my ( $leftarg, $op, $rightarg ) = @{$self}{qw{leftarg op rightarg}};
...
sub is ($$) {
    $_[0]->{lastexpected} = $_[1];
    return $_[0];
}

sub at ($$) {
    $_[0]->{lastunexpected} = $_[1];
    return $_[0];
}
...
sub new ($$$) {
    ...
    if ($::RD_AUTOACTION) {
        ...
        $self->{_check}{itempos} = $sourcecode =~ /\@itempos\b|\$itempos\s*\[/;
        $self->{_AUTOACTION} =
...
sub _no_rule ($$;$) {
    ...
    my $desc = $_[2] ? "\"$_[2]\"" : "";
    _hint(
        "You need to define a rule for the $_[0] $desc
	       to be part of."
    );
}
...
sub _generate($$$;$$) {
    ...
    for (qw(thisoffset thiscolumn prevline prevoffset prevcolumn)) {
        $self->{_check}{$_} = ( $grammar =~ /\$$_/ ) || $self->{_check}{itempos}
    ...
    while ( pos $grammar < length $grammar ) {
        ...
        {
            $code =~ s/\A\s*\(|\)\Z//g;
            _parse( "an implicit subrule", $aftererror, $line, "( $code )" );
            ...
            if ( $grammar =~ m/\G[(]/gc ) {
                ...
                elsif ( $grammar =~ m/$ANY/gco ) { $minrep = 0 }
                ...
                elsif ( $grammar =~ m/$ATLEAST/gco ) { $minrep = $1 }
                elsif ( $grammar =~ m/$ATMOST/gco )  { $maxrep = $1 }
            ...
            if ( $grammar =~ m/$OPMK/gco ) {
                ...
                # $DB::single=1;
                _parse( "a $1-associative operator directive",
                    $aftererror, $line, "<$1op:...>" );
                ...
                $item =
                  new Parse::RecDescent::Directive( '$commit=0;1', $lookahead,
                    $line, "<uncommit>" );
                $prod and $prod->additem($item)
                ...
                if ($rule) {
                    _error( "<nocheck> directive not at start of grammar",
                        $line );
                    _hint(
                ...
                if ($rule) {
                    _error( "<autotree> directive not at start of grammar",
                        $line );
                    _hint(
                    ...
                    $self->{_AUTOTREE}{TERMINAL} =
                      new Parse::RecDescent::Action(
                        q{{bless {__VALUE__=>$item[1]}, $item[0]}},
                        0, -1 );
                }
            }
                ...
                $item =
                  new Parse::RecDescent::UncondReject( $lookahead, $line,
                    "<reject>" );
                $prod and $prod->additem($item)
                  or _no_rule( "<reject>", $line );
            }
            elsif (
                $grammar =~ m/(?=$CONDREJECTMK)/gco
                and do {
              ...
              )
            {
                ...
                $code =~ /\A\s*<reject:(.*)>\Z/s;
                $item = new Parse::RecDescent::Directive( "($1) ? undef : 1",
                    $lookahead, $line, "<reject:$code>" );
                $prod and $prod->additem($item)
                  or _no_rule( "<reject:$code>", $line );
            }
            elsif (
                $grammar =~ m/(?=$SCOREMK)/gco
                and do {
                    ($code) = extract_codeblock( $grammar, '{', undef, '<' );
                    $code;
                }
...
'if ($text =~ s/\A[^\n]*\n//) { $return = 0; $& } else { undef }',
              ...
              )
            {
                ...
                $code =~ /\A\s*<resync:(.*)>\Z/s;
                $item =
                  new Parse::RecDescent::Directive( 'if ($text =~ s/\A' . $1
                      . '//) { $return = 0; $& } else { undef }',
                    $lookahead, $line, $code );
                $prod and $prod->additem($item)
                  or _no_rule( $code, $line );
            ...
            {
                _parse( "a rule variable specifier", $aftererror, $line,
                    $code );
            ...
            {
                _parse( "a deferred action specifier",
                    $aftererror, $line, $code );
              ...
              )
            {
                ...
                my $types =
                    eval 'no strict; local $SIG{__WARN__} = sub {0}; my @arr=('
                  . $code
                  . '); @arr' || ();
                if ( !$types ) {
                ...
                $item =
                  new Parse::RecDescent::Directive( '$commit = 1', $lookahead,
                    $line, "<commit>" );
                $prod and $prod->additem($item)
                ...
                $item =
                  new Parse::RecDescent::Error( '', $lookahead, $1, $line );
                $prod and $prod->additem($item)
                  or _no_rule( "<error>", $line );
                $aftererror = !$commitonly;
            }
            elsif (
                $grammar =~ m/(?=$MSGERRORMK)/gco
                and do {
                    $commitonly = $1;
                    ($code) = extract_bracketed( $grammar, '<' );
                    $code;
              ...
              )
            {
                ...
                $code =~ /\A\s*<error\??:(.*)>\Z/s;
                $item =
                  new Parse::RecDescent::Error( $1, $lookahead, $commitonly,
                    $line );
                $prod and $prod->additem($item)
                  or _no_rule( "$code", $line );
                $aftererror = !$commitonly;
            ...
            _parseunneg( "a rule declaration", 0, $lookahead, $line ) or next;
            my $rulename = $1;
            if ( $rulename =~ /Replace|Extend|Precompile|Save/ ) {
                _warn(
                    2, "Rule \"$rulename\" hidden by method
            ...
            $rule =
              new Parse::RecDescent::Rule( $rulename, $self, $line, $replace );
            ...
            pos($grammar) -= 6;
            _parseunneg( "a new (error) production",
                $aftererror, $lookahead, $line )
              or next;
            $prod->check_pending($line) if $prod;
            ...
            _parseunneg( "a new production", 0, $lookahead, $line ) or next;
            $rule
...

        {
            ...
            $grammar =~ m/(?=$MATCHRULE)/gco
            and do {
                ($code) = extract_bracketed( $grammar, '<' );
                $code;
            }
            or $grammar =~ m/$SUBRULE/gco and $code = $1
          )
        {
            my $name      = $code;
            my $matchrule = 0;
            ...
            if ( $grammar =~ m/\G[(]/gc ) {
                ...
                if ( $grammar =~ m/$OPTIONAL/gco ) {
                    ...
                    $item = new Parse::RecDescent::Repetition(
                        $name,      $1,    0,     1,
                        $lookahead, $line, $self, $matchrule,
                        $argcode
                    );
                    $prod and $prod->additem($item)
                    ...
                    _parse( "a zero-or-more subrule match",
                        $aftererror, $line, "$code$argcode($1)" );
                    if ($2) {
                        my $pos = pos $grammar;
                        ...
                        $item = new Parse::RecDescent::Repetition(
                            $name,   $1,         0,
                            $MAXREP, $lookahead, $line,
                            $self,   $matchrule, $argcode
                        );
                        $prod and $prod->additem($item)
                          or
                          _no_rule( "repetition", $line, "$code$argcode($1)" );
                    ...
                    _parse( "a $1-to-$2 subrule match",
                        $aftererror, $line, "$code$argcode($1..$2)" );
                    if ($3) {
                        my $pos = pos $grammar;
                        substr( $grammar, $pos, 0,
                    ...
                    _parse( "a one-to-$1 subrule match",
                        $aftererror, $line, "$code$argcode(..$1)" );
                    if ($2) {
                        my $pos = pos $grammar;
                        substr( $grammar, $pos, 0,
                    ...
                    _parse(
                        "an subrule match with invalid repetition specifier",
                        0, $line );
                    _error( "Incorrect specification of a repeated subrule",
                        $line );
                    _hint(
                        "Repeated subrules like \"$code$argcode$&\" cannot have
					       a maximum repetition of zero, nor can they have
					       negative components in their ranges."
                    );
                }
                ...
                $item =
                  new Parse::RecDescent::Subrule( $name, $lookahead, $line,
                    $desc, $matchrule, $argcode );
            ...
            _hint(
                "Did you mean \"|\" (to start a new production)?
        ...
        {
            _error( "Malformed action encountered", $line );
            _hint(
                "Did you forget the closing curly bracket
			       or is there a syntax error in the action?"
            );
        }
        elsif ( $grammar =~ m/$OTHER/gco ) {
            _error( "Untranslatable item encountered: \"$1\"", $line );
            _hint(
                "Did you misspell \"$1\"
			           or forget to comment it out?"
            );
        ...
        if ( $lookaheadspec =~ tr /././ > 3 ) {
            ...
            _warn(
                1, "Found two or more lookahead specifiers in a
			       row.", $line
              )
              and _hint(
    ...
    if ( $ERRORS and !_verbosity("HINT") ) {
        local $::RD_HINT = 1;
        _hint(
            'Set $::RD_HINT (or -RD_HINT if you\'re using "perl -s")
...
sub _check_insatiable($$$$) {
      ...
      )
    {
        ...
        _warn(
            3, "Subrule sequence \"$subrule($repspec) $&\" will
		       (almost certainly) fail.", $line
          )
          and _hint(
            "Unless subrule \"$subrule\" performs some cunning
...
sub _check_grammar ($) {
    ...
    foreach $rule ( values %$rules ) {
        ...
        foreach $call ( @{ $rule->{"calls"} } ) {
                ...
                && !defined &{"Parse::RecDescent::$call"} )
            {
                if ( !defined $::RD_AUTOSTUB ) {
                    _warn(
                        3, "Undefined (sub)rule \"$call\"
					      used in a production."
                      )
                      and _hint(
                        "Will you be providing this rule
        ...
        if ( $rule->isleftrec($rules) ) {
            ...
            _hint(
                "Redesign the grammar so it's not left-recursive.
...
sub _parserepeat($$$$$$$$$$)    # RETURNS A REF TO AN ARRAY OF MATCHES
{
    ...
    for ( $reps = 0 ; $reps < $max ; ) {
        ...
        if (
            !defined(
                $_tok = &$prod( $parser, $text, 1, $_noactions, $argcode )
            )
          )
        {
            $text = $_savetext;
...
sub _warn($$;$) {
    return 0
      unless _verbosity("WARN")
      && ( $::RD_HINT || $_[0] >= ( $::RD_WARN || 1 ) );
    $errortext = $_[1];
...
sub _parse($$$;$) {
    ...
    if ( $_[1] ) {
        _warn( 3, "Found $_[0] ($what) after an unconditional <error>", $_[2] )
          and _hint(
            "An unconditional <error> always causes the
